 ...
 */
 export const getFurniturePrice = (furniture) => {
  ...
  export const splitFullNameToFirstAndLast = (customer) => {
    const nameArr = customer["fullName"].split(" ");
    customer.firstName = nameArr[0];
    customer.lastName = nameArr[1];
    return customer;
  };
  ...
  export const getUserAddress = (user) => {
    ...
    const shippingLabel = `${line1} ${line2} ${city} ${postcode}`;
  ...
  export const setSafeAllergens = (customer, allergenList) => {
    ...
    const safeAllergens = allergenList.filter((allergen) => !customerAllergies.includes(allergen));

    const customersSafeAllergens = {...customer, safeAllergens};

    return customersSafeAllergens;
  };
