 ...
 */
 ...
 export const removeFalseValues = (booleanArr) => {
  ...
  export const convertStringToNumbersArray = (numberString) => {
      const splitStrings = numberString.split("+");
      const toNumbers = splitStrings.map((element) => Number(element));
    return toNumbers;
  };
  ...
  export const createOddEvenArray = (numberString) => {
      ...
      const newArr = numberArray.map((element) => element % 2 === 0 ? "even" : "odd");
    return newArr;
  };
  ...
  export const formatStringArray = (stringArr) => {
      //forEach also always returns undefined, NOT an array. Switch to map to update the array
    ...
    //This means that this method is essentially trying to read nothing
